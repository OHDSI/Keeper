---
title: "Setting KEEPER paramethers"
...
output:
  ...
  html_document:
    ...
    toc: yes
vignette: >
  %\VignetteIndexEntry{Setting Keeper paramethers}
...
This vignette describes how one sets input paramethers for Keeper to generate patient summaries.
...
First step is to create a phenotype of interest and execute it against database(s). Cohort_id of resulting cohort will be fed into KEEPER input. One can then specify a sample size to randomly select a given number of patients from the cohort (paramether sample_size) or input a comma-separated vector of person_ids to select specific patients (paramether personIds). One can further de-identify patients by replacing OMOP personId with new random ids (paramether assignNewId = TRUE).
