...
More information on creating cohorts can be found in [the Book of OHDSI](https://ohdsi.github.io/TheBookOfOhdsi/Cohorts.html).
...
We can instantiate this cohort in our database using the `CohortGenerator` package. 
First, we must specify how to connect to the server holding the CDM data, and where the cohort table will be created:
...
With a cohort and input concept sets defined, we are ready to run Keeper. 
...
Here, `conceptSets` is the data frame we created earlier holding the target concepts. 
We use `cohortDefinitionId = 1` because that was implied in our code in the [previous section](#creating-the-cohort-to-evaluate).
We specify a `sampleSize` of 20, meaning Keeper will randomly select up to 20 persons from the cohort. 
Alternatively, we could have provided a specific set of person IDs for Keeper to restrict its query to. 

We provide a `phenotypeName` that will be stored with the Keeper profiles. 
We also set `removePii = TRUE` so that the output will not contain the original person IDs or absolute dates, ensuring the output is completely anonymized with no personally identifying information (PII).
...
```{r echo = FALSE, fig.cap = "The Keeper Shiny app"}
# With R 4.6.1, knitr 1.5.2, and rmarkdown 2.32 the original ![The Keeper Shiny app](Shiny.png) could no longer find
# the image file. Only this exact code now works on all platforms:
shinyImage <- if (file.exists("Shiny.png")) "Shiny.png" else file.path("vignettes", "Shiny.png")
knitr::include_graphics(normalizePath(shinyImage, winslash = "/", mustWork = TRUE), rel_path = FALSE)
```
