...
createPromptSettings <- function(writeNarrative = TRUE,
                                 ...
                                 maxParts = 100,
                                 maxDays = 5) {
  settings <- list(writeNarrative = writeNarrative,
...
uncertaintyInstructionsTemplate <- c("In your final summary, indicate \"yes\" if the most probable scenario is that the patient had <disease>.",
...
discussEvidenceFormatTemplate <- c("Evidence in favor of <disease>:",
                                   "",
...
createSystemPrompt <- function(settings, diseaseName) {
  ...
  if (settings$provideExamples) {
    ...
    prompt <- c(prompt, "Healthcare data:",
                "",
...
createPrompt <- function(settings,
  ...
  prompt <- c(prompt, sprintf("Demographics and details about the visit: %s, %s yo; Visit: %s",
                              keeperRow$gender,
                              convertAgeToText(keeperRow$age),
                              formatVisitContext(keeperRow$visitContext)))
  prompt <- c(prompt, sprintf("Diagnoses recorded on the day of the visit: %s",
                              formatPresentation(keeperRow$presentation,
                                                 maxParts = settings$maxParts)))
  prompt <- c(prompt, sprintf("Diagnoses recorded prior to the visit: %s",
                              formatList(keeperRow$priorDisease, keeperRow$symptoms, keeperRow$comorbidities,
                                         maxParts = settings$maxParts,
                                         maxDays = settings$maxDays)))
  prompt <- c(prompt, sprintf("Treatments recorded prior to the visit: %s",
                              formatList(keeperRow$priorDrugs, keeperRow$priorTreatmentProcedures,
                                         maxParts = settings$maxParts,
                                         maxDays = settings$maxDays)))
  prompt <- c(prompt, sprintf("Diagnostic procedures recorded proximal to the visit: %s",
                              formatList(keeperRow$diagnosticProcedures,
                                         maxParts = settings$maxParts,
                                         maxDays = settings$maxDays)))
  prompt <- c(prompt, sprintf("Laboratory tests recorded proximal to the visit: %s",
                              formatList(keeperRow$measurements,
                                         maxParts = settings$maxParts,
                                         maxDays = settings$maxDays)))
  prompt <- c(prompt, sprintf("Alternative diagnoses recorded proximal to the visit: %s",
                              formatList(keeperRow$alternativeDiagnosis,
                                         maxParts = settings$maxParts,
                                         maxDays = settings$maxDays)))
  prompt <- c(prompt, sprintf("Diagnoses recorded after the visit: %s",
                              formatList(keeperRow$afterDisease,
                                         maxParts = settings$maxParts,
                                         maxDays = settings$maxDays)))
  prompt <- c(prompt, sprintf("Treatments recorded during or after the visit: %s",
